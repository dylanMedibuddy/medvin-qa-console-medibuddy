...
const ALLOWED = ['patched', 'patch_error', 'dry_run_would_patch'] as const
type PatchStatus = (typeof ALLOWED)[number]
...
export const PATCH = withMakeAuth<Ctx>(async (request: NextRequest, ctx: Ctx) => {
  ...
  const fields: string[] = []
  if (typeof body.status !== 'string' || !ALLOWED.includes(body.status as PatchStatus)) {
    fields.push('status')
  }
  if (
  ...
  const newStatus = body.status as PatchStatus
